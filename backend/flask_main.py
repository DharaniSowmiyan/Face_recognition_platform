...
# --- Utility Functions (same as your logic) ---
...
def check_haarcascadefile():
    if not os.path.isfile("haarcascade_frontalface_default.xml"):
        raise FileNotFoundError("Haarcascade file missing!")
                ...
                if conf < 50:
                    ...
                    if data:
                        ...
                        cursor.execute("INSERT INTO Attendance (student_id, name, date, time) VALUES (?, ?, ?, ?)",
                                       (student_id, name, date_str, timeStamp))
                        conn.commit()
                        attendance_records.append({'id': student_id, 'name': name, 'date': date_str, 'time': timeStamp})
...
# Add this function to generate frames with bounding boxes
def gen_frames():
    check_haarcascadefile()
    cam = cv2.VideoCapture(0)
    harcascadePath = "haarcascade_frontalface_default.xml"
...
def gen_frames_for_scan():
    ...
    conn = sqlite3.connect('FaceRecog.db')
    cursor = conn.cursor()
    already_marked = set()  # To avoid duplicate entries in one scan session
    ...
    while scanning:
        ...
        for (x, y, w, h) in faces:
            ...
            if conf < 50:
                ...
                if data:
                    name, student_id = data
                    # Only mark attendance once per scan session per person
                ...
                name = "Unknown"
            cv2.rectangle(frame, (x, y), (x + w, y + h), (0, 255, 0), 2)
            cv2.putText(frame, name, (x, y + h + 30), cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 255, 0), 2)
        ret, buffer = cv2.imencode('.jpg', frame)
        frame = buffer.tobytes()
        yield (b'--frame\r\n'
               b'Content-Type: image/jpeg\r\n\r\n' + frame + b'\r\n')
